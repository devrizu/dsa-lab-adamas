...
int main() {
    ...
    if (fgets(input, sizeof(input), stdin) != NULL) {
        ...
        else if (strcmp(input, "stack") == 0 || strcmp(input, "queue") == 0) {
            printf("%s is a Non-Primitive, Linear abstract data structure (can be static or dynamic).\n", original_input);
        } 
        else if (strcmp(input, "tree") == 0 || strcmp(input, "graph") == 0) {
            printf("%s is a Non-Primitive, Non-Linear data structure.\n", original_input);
        } 
        else if (strcmp(input, "hash table") == 0 || strcmp(input, "hash map") == 0) {
            printf("%s is a Non-Primitive, Non-Linear data structure.\n", original_input);
        } 
        else if (strcmp(input, "integer") == 0 || strcmp(input, "int") == 0 || 
                 strcmp(input, "float") == 0 || strcmp(input, "char") == 0 || 
                 strcmp(input, "boolean") == 0 || strcmp(input, "bool") == 0) {
            printf("%s is a Primitive data structure.\n", original_input);
        } 
        else {
            printf("Classification for '%s' is not found. Please try standard names like Array, Tree, or Stack.\n", original_input);
        }
    }
